...
func addOneoffRequestFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVarP(&flags.VarPrefix, "var-prefix", "p", "$", "Set the leading variable symbol used to indicate the start of a variable in the request to `SYM`.")
...
func makeOneoffRequest(io cmdio.IO, args oneoffArgs) error {
	...
	_, err := morc.Send(strings.ToUpper(args.method), args.url, "$", sendOpts)
	return err
}
